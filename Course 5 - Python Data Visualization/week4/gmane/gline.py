...
messages = dict()
for message_row in cur :
    if (message_row[4] is None): continue
    print("message brut: ", message_row[4])
    print(re.findall('^([0-9]+)\-.+$', message_row[4])[0])
    messages[message_row[0]] = (message_row[1],message_row[2],message_row[3], re.findall('^([0-9]+)\-.+$', message_row[4])[0])
